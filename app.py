...
def load_theme():
    ...
    /* YENİ: HABER BANDI (NEWS TICKER) CSS */
    .ticker-wrap {
        width: 100%;
        ...
        margin-top: -3rem; /* Streamlit'in üst boşluğunu sıfırlamak için */
        margin-bottom: 20px;
    }
    .ticker-text {
        display: inline-block;
...
if st.session_state.page == 'landing':
    # Landing sayfasında da dil değiştirebilsin diye sağ üste ufak bir radyo butonu
...
# ==============================================================================
    ...
    # ---------------- EN ÜST: HABER BANDI (NEWS TICKER) ----------------
    ticker_text_formatted = t['ticker_template'].replace("{symbol}", SYMBOL_LABELS[symbol])
    st.markdown(f"""
        <div class="ticker-wrap">
            <div class="ticker-text">{ticker_text_formatted}</div>
        </div>
    """, unsafe_allow_html=True)
